import { useRef } from "react";
import type { BallotItem } from "@/types/ballot";
import BallotAnnotation from "@/components/BallotAnnotation";
import { cn } from "@/lib/utils";

interface BallotMobileListProps {
  items: BallotItem[];
  hoveredIndex: number | null;
  onToggleIndex: (index: number) => void;
}

export default function BallotMobileList({
  items,
  hoveredIndex,
  onToggleIndex,
}: BallotMobileListProps) {
  return (
    <div className="space-y-3">
      {items.map((item, index) => (
        <ExpandableItem
          key={item.id}
          item={item}
          isOpen={hoveredIndex === index}
          onToggle={() => onToggleIndex(index)}
        />
      ))}
    </div>
  );
}

function ExpandableItem({
  item,
  isOpen,
  onToggle,
}: {
  item: BallotItem;
  isOpen: boolean;
  onToggle: () => void;
}) {
  const contentRef = useRef<HTMLDivElement>(null);

  return (
    <div className="space-y-0">
      <button
        className={cn(
          "w-full text-left px-4 py-3 bg-card border border-border transition-[border-radius] duration-300",
          isOpen ? "rounded-t-lg" : "rounded-lg",
        )}
        onClick={onToggle}
      >
        <div className="flex items-start gap-3">
          <div className="flex-shrink-0 mt-0.5">
            <div
              className={cn(
                "w-4 h-3 rounded-[50%] border-[1.5px] flex items-center justify-center transition-all",
                isOpen
                  ? "border-foreground/70 bg-foreground/70"
                  : "border-foreground/25",
              )}
            >
              {isOpen && (
                <svg className="w-2 h-1.5 text-card" viewBox="0 0 12 10" fill="none">
                  <path d="M1 5L4 8L11 1" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              )}
            </div>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">
              {item.category === "office" ? "Elected Office" : item.category}
            </p>
            <h3 className="font-display text-sm font-semibold text-foreground leading-snug">
              {item.title}
            </h3>
          </div>
        </div>
      </button>

      {/* Smooth height-animated panel */}
      <div
        className="overflow-hidden transition-[height] duration-[800ms] ease-[cubic-bezier(0.25,0.1,0.25,1)]"
        style={{ height: isOpen ? contentRef.current?.scrollHeight ?? 0 : 0 }}
      >
        <div ref={contentRef} className="border border-t-0 border-border rounded-b-lg">
          <BallotAnnotation item={item} isActive={true} animateText={isOpen} />
        </div>
      </div>
    </div>
  );
}
